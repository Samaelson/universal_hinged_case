...
module wall_body(x_h=50,y_h=100,z_h=50){

    z_cut = tan((90-atan2(y_h,z_h))/3)*y_h+8 > 20 ? 20 : tan((90-atan2(y_h,z_h))/3)*y_h+8; 

    a_cut = (90-atan2(y_h,z_cut))/2;
    ...
    difference(){
        union(){
            ...
            difference(){
                ...
                translate([-(x_h+17)/2,y_h-(y_h+13)/2,3])
                rotate([-a_cut,0,0])
                translate([0,-y_h,0])
                cube([x_h+17, y_h+13,2*z_h]);        
        ...
        translate([-(x_h+50)/2, -(y_h+10)/2,z_cut])
...
module lid_body(x_h=50,y_h=100,z_h=50){
    ...
    difference(){
        union(){
            ...
            difference(){
                ...
                translate([-(x_h+17)/2,y_h-(y_h+17)/2,-40+3-11])
                rotate([-a_cut,0,0])
                translate([0,-y_h,0])
                cube([x_h+17, y_h+13,40]);     
...
module case_body(x_o=50,y_o=100,z_o=50,x_d=20,y_d=20,z_d=5){

    x_h = x_o+2*x_d+t;
    y_h = y_o+2*y_d+t+tan(2)*(x_h/2);
    z_h = z_o+z_d+t;
   
    echo("Case size: x=",x_h,"y=",y_h,"z=",z_h);
    
    difference(){
        wall_body(x_h,y_h,z_h);
 
        // Magnet cutout
//https://www.magnetladen.de/quadermagnet-20x10x2-mm-n45-nickel/?gad_source=1&gad_campaignid=17542606441&gbraid=0AAAAAD_IURFxJtm_5yEXzVLneTMiy88LD&gclid=EAIaIQobChMIwpjJsbKfkgMVd5qDBx3EgDTzEAQYByABEgKD4_D_BwE
...
//   z_d = d distance between object and lid.
//     a = lid opening angle.
module case_lid(x_o=50,y_o=100,z_o=50,x_d=20,y_d=20,z_d=5,a=0){
